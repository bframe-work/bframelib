...
class TestPriceSpans:
    ...
    def test_usage_price_spans(self, client: Client):
        ...
        price_spans = df.to_dict('records')
        ...
        for month in range(1, 13):
            ...
            start_of_month_dt = datetime.datetime(year=YEAR, month=month, day=1, tzinfo=datetime.timezone.utc)
            next_month_dt = datetime.datetime(year=YEAR, month=month, day=last_day, tzinfo=datetime.timezone.utc) + datetime.timedelta(days=1)
    ...
    def test_fixed_price_spans(self, client: Client):
        ...
        ended_at = datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc)
    ...
    def test_event_price_spans_on_mm_contract_change_scd(self, client: Client):
        ...
        df = res.df()
        apply_utc_on_cols(df)
        price_span = df.to_dict('records')[0]

        # Check price span
        assert price_span['started_at'] == datetime.datetime(2023, 12, 1, tzinfo=datetime.timezone.utc)
        assert price_span['ended_at'] == datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc)
        assert price_span['effective_at'] == datetime.datetime(2023, 12, 11, tzinfo=datetime.timezone.utc)
        assert price_span['ineffective_at'] == datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc)
    ...
    def test_fixed_price_spans_on_mm_contract_change_scd(self, client: Client):
        ...
        assert price_span['started_at'] == datetime.datetime(2023, 11, 1, tzinfo=datetime.timezone.utc)
        assert price_span['ended_at'] == datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc)
        assert price_span['effective_at'] == datetime.datetime(2023, 11, 1, tzinfo=datetime.timezone.utc)
        assert price_span['ineffective_at'] == datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc)
        assert price_span['proration_start'] == datetime.datetime(2023, 11, 1, tzinfo=datetime.timezone.utc)
        assert price_span['proration_end'] == datetime.datetime(2024, 11, 1, tzinfo=datetime.timezone.utc)
        assert price_span['price'] == '1000'
        assert price_span['product_type'] == 'FIXED'
        assert price_span['product_uid'] == 2
        assert price_span['pricebook_id'] == '10'
    ...
    def test_price_spans_on_offset_dates_and_quarterly_invoice_schedule(self, client: Client):
        ...
        assert start_price_span['started_at'] == datetime.datetime(2023, 1, 2, tzinfo=datetime.timezone.utc)
        assert start_price_span['ended_at'] == datetime.datetime(2023, 4, 1, tzinfo=datetime.timezone.utc)
        assert start_price_span['effective_at'] == datetime.datetime(2023, 1, 2, tzinfo=datetime.timezone.utc)
        assert start_price_span['ineffective_at'] == datetime.datetime(2023, 4, 1, tzinfo=datetime.timezone.utc)
        ...
        assert end_price_span['started_at'] == datetime.datetime(2023, 10, 1, tzinfo=datetime.timezone.utc)
        assert end_price_span['ended_at'] == datetime.datetime(2023, 12, 7, tzinfo=datetime.timezone.utc)
        assert end_price_span['effective_at'] == datetime.datetime(2023, 10, 1, tzinfo=datetime.timezone.utc)
        assert end_price_span['ineffective_at'] == datetime.datetime(2023, 12, 7, tzinfo=datetime.timezone.utc)
    ...
    def test_price_spans_on_offset_one_time_charge(self, client: Client):
        ...
        assert price_span['started_at'] == datetime.datetime(2023, 2, 1, tzinfo=datetime.timezone.utc)
        assert price_span['ended_at'] == datetime.datetime(2023, 12, 7, tzinfo=datetime.timezone.utc)
        assert price_span['effective_at'] == datetime.datetime(2023, 2, 1, tzinfo=datetime.timezone.utc)
        assert price_span['ineffective_at'] == datetime.datetime(2023, 12, 7, tzinfo=datetime.timezone.utc)
    ...
    def test_proration_range_for_advanced_price_spans(self, client: Client):
        ...
        assert price_span['proration_start'] == datetime.datetime(2023, 1, 1, tzinfo=datetime.timezone.utc)
        assert price_span['proration_end'] == datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc)

        # This is just checking everyting else functions as normal
        assert price_span['started_at'] == datetime.datetime(2023, 1, 2, tzinfo=datetime.timezone.utc)
        assert price_span['ended_at'] == datetime.datetime(2023, 11, 7, tzinfo=datetime.timezone.utc)
        assert price_span['effective_at'] == datetime.datetime(2023, 1, 2, tzinfo=datetime.timezone.utc)
        assert price_span['ineffective_at'] == datetime.datetime(2023, 11, 7, tzinfo=datetime.timezone.utc)
        assert price_span['price'] == '1000'
        assert price_span['product_type'] == 'FIXED'
        assert price_span['product_uid'] == 2
    ...
    def test_proration_range_for_arrears_price_spans(self, client: Client):
        ...
        # The proration range should cover the entire months that are supposed to be allocated based on the invoice schedule
        # This allows us the proration to work as intended
        assert first_price_span['proration_start'] == datetime.datetime(2023, 1, 1, tzinfo=datetime.timezone.utc)
        assert first_price_span['proration_end'] == datetime.datetime(2023, 3, 1, tzinfo=datetime.timezone.utc)
        assert first_price_span['started_at'] == datetime.datetime(2023, 1, 2, tzinfo=datetime.timezone.utc)
        assert first_price_span['ended_at'] == datetime.datetime(2023, 3, 1, tzinfo=datetime.timezone.utc)
        assert first_price_span['effective_at'] == datetime.datetime(2023, 1, 2, tzinfo=datetime.timezone.utc)
        assert first_price_span['ineffective_at'] == datetime.datetime(2023, 3, 1, tzinfo=datetime.timezone.utc)

        # Test on the other end of the spectrum
        assert last_price_span['proration_start'] == datetime.datetime(2023, 11, 1, tzinfo=datetime.timezone.utc)
        assert last_price_span['proration_end'] == datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc)
        assert last_price_span['started_at'] == datetime.datetime(2023, 11, 1, tzinfo=datetime.timezone.utc)
        assert last_price_span['ended_at'] == datetime.datetime(2023, 11, 7, tzinfo=datetime.timezone.utc)
        assert last_price_span['effective_at'] == datetime.datetime(2023, 11, 1, tzinfo=datetime.timezone.utc)
        assert last_price_span['ineffective_at'] == datetime.datetime(2023, 11, 7, tzinfo=datetime.timezone.utc)
    ...
    def test_fixed_contract_price_spans(self, client: Client):
        ...
        dates = (
            (datetime.datetime(2023, 1, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 5, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 5, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 9, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 9, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc))
        )
    ...
    def test_contract_solo_contract_price_spans(self, client: Client):
        ...
        dates = (
            (datetime.datetime(2023, 1, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 4, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 4, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 7, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 7, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 10, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 10, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc))
        )
    ...
    def test_price_schedule_part_one(self, client: Client):
        ...
        dates = (
            (datetime.datetime(2023, 1, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 2, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 2, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 3, 1, tzinfo=datetime.timezone.utc))
        )
    ...
    def test_price_schedule_part_two(self, client: Client):
        ...
        dates = (
            (datetime.datetime(2023, 3, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 4, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 4, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 5, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 5, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 6, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 6, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 7, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 7, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 8, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 8, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 9, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 9, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 10, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 10, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 11, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 11, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 12, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 12, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc))
        )
    ...
    def test_contract_with_contract_price_started_and_ended_at(self, client: Client):
        ...
        assert price_span['started_at'] == datetime.datetime(2023, 6, 1, tzinfo=datetime.timezone.utc)
        assert price_span['ended_at'] == datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc)
        assert price_span['proration_start'] == datetime.datetime(2023, 6, 1, tzinfo=datetime.timezone.utc)
        assert price_span['proration_end'] == datetime.datetime(2024, 6, 1, tzinfo=datetime.timezone.utc)
    ...
    def test_pricebook_slowly_changing_dimensions_event_product(self, client: Client):
        ...
        expected_price_spans = (
            (datetime.datetime(2023, 1, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 2, 1, tzinfo=datetime.timezone.utc), '0.33'),
            (datetime.datetime(2023, 2, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 3, 1, tzinfo=datetime.timezone.utc), '0.33'),
            (datetime.datetime(2023, 3, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 4, 1, tzinfo=datetime.timezone.utc), '0.33'),
            (datetime.datetime(2023, 4, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 5, 1, tzinfo=datetime.timezone.utc), '0.33'),
            (datetime.datetime(2023, 7, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 10, 1, tzinfo=datetime.timezone.utc), '0.22'),
            (datetime.datetime(2023, 10, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc), '0.22'),
        )
    ...
    def test_pricebook_slowly_changing_dimensions_fixed_product(self, client: Client):
        ...
        assert price_span['started_at'] == datetime.datetime(2023, 1, 1, tzinfo=datetime.timezone.utc)
        assert price_span['ended_at'] == datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc)
        assert price_span['price'] == '1000'
        assert price_span['product_type'] == 'FIXED'
        assert price_span['product_uid'] == 2
    ...
    def test_long_dated_contract_with_offset_start_date_and_7_month_invoice_schedule(self, client: Client):
        ...
        dates = (
            (datetime.datetime(2021, 7, 10, tzinfo=datetime.timezone.utc), datetime.datetime(2022, 2, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2022, 2, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2022, 9, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2022, 9, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 4, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 4, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2023, 11, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2023, 11, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2024, 6, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2024, 6, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2025, 1, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2025, 1, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2025, 8, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2025, 8, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2026, 3, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2026, 3, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2026, 10, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2026, 10, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2027, 5, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2027, 5, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2027, 12, 1, tzinfo=datetime.timezone.utc)),
            (datetime.datetime(2027, 12, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2028, 1, 1, tzinfo=datetime.timezone.utc)),
        )
...
class TestPriceSpansDateRange:
    def test_base_case(self, client: Client):
        ...
        assert min_date == datetime.datetime(2021, 7, 1, tzinfo=datetime.timezone.utc)
        assert max_date == datetime.datetime(2036, 1, 1, tzinfo=datetime.timezone.utc)
    
    def test_specific_range(self, client: Client):
        client.set_config({'rating_range': ['2023-01-01', '2023-02-01']})
        min_date, max_date = client.get_price_span_date_range(('EVENT', 'FIXED'))
        assert min_date == datetime.datetime(2023, 1, 1, tzinfo=datetime.timezone.utc)
        assert max_date == datetime.datetime(2024, 1, 1, tzinfo=datetime.timezone.utc)
