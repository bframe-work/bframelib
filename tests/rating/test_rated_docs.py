...
class TestRatedDocumentsDateTimeTarget:
    def test_current_read_mode(self, client: Client):
        client.set_config({
            'read_mode': 'CURRENT',
            'lookback_window': 3,
            'forward_window': 0,
    ...
    def test_unsaved_current_read_mode(self, client: Client):
        client.set_config({'read_mode': 'CURRENT'})
        ...
        client.set_config({'read_mode': 'UNSAVED_CURRENT'})
        client.execute("""
    ...
    def test_changes_to_pricing(self, client: Client):
        ...
        # Set to current and update pricing
        client.set_config({
            'read_mode': 'CURRENT',
        })
